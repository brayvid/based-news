...
try:
    ...
    # We need a basic logger here in case the main setup fails
...
# Create a file handler to write to logs/experiment.log
    ...
    try:
        ...
        if candidate.finish_reason.name != "STOP":
            logging.warning(
                ...
                f"Reason: {candidate.finish_reason.name}. Safety Ratings: {candidate.safety_ratings}"
            )
        ...
        logging.error(f"{log_prefix}: Failed to parse JSON from Gemini response: {jde}. Raw response text: '{raw_text}'")
...
def check_prediction_vacuousness_gemini(prediction_text, context_summary_text, report_date_str):
    prompt = f"""
    You are an AI assistant evaluating the quality of a geo-political/economic prediction.
    Your task is to assess if the given prediction is specific and testable, or if it is too vague, obvious, or untestable.

    A "vacuous" prediction:
    1. States something almost always true (e.g., "markets will fluctuate").
    2. Lacks specific actors, actions, or measurable outcomes.
    3. Merely rephrases an ongoing situation from the provided context without predicting a distinct new development.

    Prediction: "{prediction_text}"

    Context from week ending {report_date_str}:
    ---
    {context_summary_text[:4000]} 
    ---

    Your output MUST be a single JSON object with the following keys:
    - "is_obvious_or_vacuous": boolean
    - "obviousness_score": float (0.0 for highly specific, 1.0 for extremely obvious)
...
def get_prior_likelihood_from_gemini(prediction_text, context_summary_text, report_date_str):
    ...
    Based ONLY on the provided context (events from the week ending {report_date_str}) and general knowledge up to that date, estimate the probability of the prediction coming true in the next 1-2 weeks.
    ...
    Your output MUST be a single JSON object with the following keys:
    - "prior_probability": float (from 0.0 to 1.0)
...
def main():
    ...
    if current_summaries_fingerprint == last_run_summaries_fingerprint and os.path.exists(EVALUATED_PREDICTIONS_FILE):
        logging.info(f"No changes in {SUMMARIES_FILE} since last run. Exiting.")
        return
    ...
    for i, report_n in enumerate(all_summaries):
        ...
        for pred_text in report_n_predictions_list:
            ...
            if not _gemini_check_previously_attempted_and_concluded(current_eval_entry.get("vacuousness_check")):
                ...
                if "error" in vac_check_result:
                    current_eval_entry["status"] = "error_vacuousness_check"
                elif vac_check_result.get("is_obvious_or_vacuous"):
            ...
            if current_eval_entry["status"] == "pending_prior_estimation" and not _gemini_check_previously_attempted_and_concluded(current_eval_entry.get("prior_likelihood")):
                ...
                if "error" in prior_result:
                    current_eval_entry["status"] = "error_prior_estimation"
                else:
            ...
            if current_eval_entry["status"] in ["pending_verification", "verified_not_occurred"]:
                ...
                if "verification_details" not in current_eval_entry or not isinstance(current_eval_entry.get("verification_details"), dict):
                    current_eval_entry["verification_details"] = {}
                
                verification_dict = current_eval_entry["verification_details"]
                verification_dict.setdefault("checked_against_reports", [])
                verification_dict.setdefault("evidence_report_timestamp", None)
                verification_dict.setdefault("match_score", 0.0)
                ...
                for j in range(i + 1, len(all_summaries)):
                    ...
                    if report_n_plus_k_timestamp_str in verification_dict["checked_against_reports"]:
                        continue

                    verification_dict["checked_against_reports"].append(report_n_plus_k_timestamp_str)
                    this_pred_entry_modified_in_run = True
                    ...
                    if occurred and score > verification_dict["match_score"]:
                        ...
                        verification_dict["evidence_report_timestamp"] = report_n_plus_k_timestamp_str
                        verification_dict["match_score"] = round(score, 3)
                        verification_dict["matching_keywords"] = common_kws
                        if score > 0.4: break
                ...
                if current_eval_entry["status"] != original_status:
                    this_pred_entry_modified_in_run = True
                
                evaluated_predictions_dict[pred_id] = current_eval_entry

            if this_pred_entry_modified_in_run:
                made_eval_data_changes_in_run = True
...
def run_analysis(predictions):
    ...
    final_predictions = [p for p in predictions if p['status'] not in [
        "filtered_obvious", "pending_processing", "pending_prior_estimation",
        "error_vacuousness_check", "error_prior_estimation"
    ]]
    ...
    preds_for_brier = [
        p for p in (verified_occurred + verified_not_occurred)
...
if __name__ == "__main__":
    try:
        ...
        logging.exception("An unhandled exception occurred during main execution.")
        sys.exit(1)